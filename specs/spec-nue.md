...
- "キーボードから手を離さない" ナビゲーション体験の実現を提供する。キーボードショートカットやコマンドパレット（Command Hub）との高度な連携により内部的なアクションはマウスを持ち直す必要がない。
...
* **テーマ**: 「Neon-Night-Glass」（深いネイビーを基調に、高彩度なネオンカラーと、背後の情報を有機的に遮断・透過させる「くすみガラス（Frosted Glass）」によって構成される）。
...
| カテゴリ | 色名 | HEX | 主な配置 | 備考 |
| :--- | :--- | :--- | :--- | :--- |
| **Background** | **Deep Abyss** | `#0B0E14` | 最背面。エディタ/Galaxy View。 | すべての「光」を受け止める無の空間。 |
| **Glass (Blur)** | **Atmosphere** | `#1A1D23` | Command Hub、サイドパネル背景。 | **Alpha: 0.7 + Blur: 20px**。背後を透過・拡散させる。 |
| **Surface** | **Space Grey** | `#242933` | アクティブなタブ、ボタンの基底。 | ガラス層より一段不透明度を上げ、操作対象を明確化。 |
| **Border** | **Midnight Glass** | `#3D4455` | パネル境界線、Specular Edge。 | ガラスの縁に 0.5px の鋭い光沢を与える。 |
| **Primary (AI)** | **Neon Cyan** | `#00F5FF` | AI活動、ステータス、現在行。 | ガラス越しに最も美しく滲む、Nueのシグネチャー。 |
| **Success** | **Electric Lime** | `#32FF7E` | 承認済、保存完了。 | 視認性の高い発光グリーン。 |
| **Warning** | **Solar Flare** | `#FFF200` | 承認待ち、未承認差分（Gutter）。 | 霧（ガラス）の中で最も膨張して見える警告色。 |
| **Error** | **Cyber Magenta** | `#FF006E` | ビルドエラー、異常振動、拒否。 | 強い彩度でユーザーの視覚に割り込む。 |
| **Information** | **Ether Purple** | `#BF5AF2` | LSP 型情報、ドキュメントリンク。 | 他の色と干渉しにくい中性的な補助色。 |
| **Text (Main)** | **Cloud White** | `#F8F9FA` | コード、ラベル。 | ガラスの背後がどんな色でも読める、高い輝度。 |
| **Text (Muted)** | **Dusty Grey** | `#949DB1` | コメント、非アクティブUI。 | 背景に沈み込みつつ、必要十分な可読性を維持。 |

各色は直線的に使い回しを避け、視覚上の階層を保持するために `Neon Cyan` / `Cyber Magenta` / `Solar Flare` 等の高彩度色は限定的なハイライトやステータス表示にのみ使用することを **SHOULD** とする。

質感の３大原則
- Backdrop Blur (背景ぼかし): パネル背後のコードやGalaxyのノードを、ガウスぼかしで抽象化。情報の連続性を保ちつつ、前面のUIを際立たせる。
- Fine Grain (微細なノイズ): ガラス表面にわずかなデジタル・ノイズを重畳。これにより、単なる「半透明」ではない、物理的な「すりガラス」の物質感と高級感を与える。
- Specular Edge (鏡面反射の縁): パネルの境界線に、光源（ネオン）からの反射を模した 0.5px〜1px の非常に細いシアンの線 を配置。
