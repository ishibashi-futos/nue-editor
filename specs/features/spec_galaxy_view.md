...
### 3.3.1 Galaxy View の大規模スコアリング

`Galaxy View` が 500 ノードを超えるような大規模状態になると、描画の複雑化と認知負荷の増大から詳細度を自動的に低減する。ユーザーの「現在の関心事」を軸に選別するため、次のスコアリング基準を適用し、スコア順に優先して描画を維持することでノード数を削減する。

- **P0: Active / AI Activity** ― 現在エージェントが編集中のファイル（彗星で表示）およびその直近の依存先。最も高いスコアを与え、必ず描画対象とする。
- **P1: Focus / User Focus** ― ユーザーが開いているタブ、カーソルがあるファイル、Command Hub で選択中の候補に絡むノード。P0 の次に保持し、アニメーションやハイライトで強調する。
- **P2: Impact / Dependency Hub** ― 依存関係（エッジ）が集中する中心的モジュールや CLI 等の共通基盤。変更の波及力が大きいため、視覚化して被依存ノードとの接続を示す。
- **P3: Context / Distance** ― P0/P1 からのグラフ距離が 1 以内のノード。文脈を補完するが、必要でない場合はスコアの低い順に描画を抑制する。

スコアの合計が上位になるノードから描画領域を確保し、500 ノードを超えた残りは自動的に折りたたむ。表示オミット時には、状態変化や差分の発生があれば即座に再スコアリングして描画対象を入れ替える。ユーザーによる手動制御（展開・折りたたみボタン）を設けず、スコアリングが自律的に描画維持/削減を決定することを **SHOULD** とする。これにより `Galaxy View` の詳細度低下が常に現在の関心事に即して行われ、ユーザーオーバーヘッドを最小化する。

### 3.3.2 Nebula クラスタリングと表示制御

低スコアのノード群は同一ディレクトリ内で束ねられ、「Nebula（星雲）」という集合として描画する。Nebula は通常のノードを表示せず、ディレクトリ単位の大きな発光体（ディフューズなグロー）として描くことで視覚コストを下げる。Nebula に含まれる、明確に修正対象でない node_modules / vendor / 外部ライブラリ等は `Galaxy View` の描画対象から除外し、ユーザーの操作で追跡しない限り現れない。Nebula への展開やダブルクリックなどのジェスチャーは初期段階では提供せず、描画状態の同期のみを保証する。

### 3.3.3 表示パフォーマンスとレイアウト制御

`Galaxy View` は常に現行フォーカス周辺のノードのみを物理シミュレーションの対象とし、遠方のノードは座標を固定（フリーズ）しておくことで、全ノードに対する毎フレームのレイアウト計算を避ける。GPUI のインスタンシング描画を活用し、同一形状のノード（星）やエッジ（光線）をバッチ化して描画コールを削減する。フォーカスノードの再描画では逐次的に追加/削除を行い、再レイアウトのタイムスライスを 16ms 以下に収める。

これらの処理は、ユーザー固有の関心事（P0～P3）に従って描画対象を動的に入れ替える `Galaxy View Prioritization Loop` として定義し、描画の欠落・再構築・イベントの同期を `Audit Event` に記録する。パフォーマンスが損なわれた場合は、`Galaxy View` も `Command Hub` 同様に `Backoff` 状態を表示して `Re-scoring…` ラベルを出し、最新スコアの再評価が完了したタイミングで強調ノードを更新することを **SHOULD** とする。
