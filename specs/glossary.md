...
| Atomic Intent | 概念 | 自然言語入力から生成された複数の `focus_id` を含む候補を「論理的最小単位」でひとまとめにしたまとまり。`Command Hub` は `parent_intent_id` を付与しつつ差分を集約して表示し、`Shadow Buffer`/`Audit Event` で `focus_id` ごとの承認状態も追跡できるようにする。 | `spec-nue.md` Sec.6.1.1 で `parent_intent_id` と `Atomic Intent` に基づく `Approval Unit` の扱いを RFC 2119 で定義した。 |
| Relevance Intent | 機能 | `Semantic Search` 結果に紐づけられた意図トークンで、`Command Hub` への `Intent Request` と `Approval Unit` の橋渡しを担う。`semantic_score`/`focus_ids`/`parent_intent_id`/`approval_state`/`related_audit_event_id` を含み、`Audit Event` への記録と `Shadow Buffer` の `focus_id` 同期を保証する。 | `spec-nue.md` Sec.6.2.2 で `Semantic Match` から `Command Hub` へ送る流れ、`Relevance Intent` の構造、`Command Hub` が `Intent Request` に埋め込む `parent_intent_id` の利用を追記した。 |
| Color-limited Mode | UI | 高彩度制限や色覚制約などにより通常のカラーパレットが使えない状況。色以外の `Glyph`/テキスト/ラベルを併用し、モード切り替えトリガーが確定したら代替表現に即時変換する柔軟性を保つことを要件とする。 | `spec-nue.md` Sec.3.1 に `Glyph` 併用を求める追記をし、`specs/ask.md` Q29 でトリガー仕様を確認する予定。 |
